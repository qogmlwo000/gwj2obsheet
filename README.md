...
| 행 다중 선택 (드래그) | 셀 아무 곳이나 누른 채 아래/위 행으로 드래그 |
| 대량 등록 (2,000명+) | DATA 탭 → `📗 엑셀` → 템플릿 다운로드 → 작성 → 가져오기 |
...
| 관리자 (`Bennett` + 설정에서 추가한 닉네임) | 위 + 행 삭제 + 카테고리 비우기 + DAY/SWING 백업·복원·초기화 + 마감시간 관리 + TC 포지션 편집 + 사원 특이사항 + **CSV 내보내기** + **관리자 추가/해제** |

> 관리자 추가: ⚙ 설정 → **⚡ 관리자** 섹션에서 닉네임 등록. 등록된 닉네임은 DATA 등록 없이도 바로 입장 가능하며 관리자 권한을 갖습니다. `Bennett` 은 항상 고정 관리자입니다.
...
- 🔴 **FLOW 쿠코드 입력 버그 수정** — 셀 포커스 시 자동완성 아이콘이 셀 전체에 문양처럼 타일링되고 드롭다운이 가려지던 문제 (`:focus` 의 `background` 단축 속성이 원인) 해결
- 🟢 **DATA — CD 카테고리 추가** (다른 공정 지원): 쿠코드·성함·공정(IB/CFC/5OB/HUB/ICQA)·비고. 라벨은 `성함 - 공정 CD` (예: `홍길동 - IB CD`)
- 🟢 **DATA 특수에 메뉴얼 멀티 / 오토백 멀티 추가** — 메뉴얼 멀티 가능자는 M/A/P 의 M이 **주황색**으로 강조. 오더피커 표시는 `- OP` 로 단축
- 🟢 **PICK 라벨 정비** — `7.1F (AGV)` / `7.1F (AS/HV)`, `8F · ES` (구 8.1)
- 🟢 **공유 PICK** — 6.1F / 6.3F 를 **6F** 로 통합, 보드는 이름만 깔끔하게(하이스킬 색 제거), **Perm(계약직)만** 자동 추가 (TEMP·캡틴 제외)
- 🟢 **복사 2종** — 모든 그리드 선택바/우클릭에서 `쿠코드 | 성함` 과 `쿠코드만` 선택 복사 (Ctrl+C 는 쿠코드|성함)
- 🟢 **검색 강조 개선** — 검색 시 매칭 안 되는 행은 어둡게, 매칭 행만 환하게. 콤마(,)로 여러 명 동시 검색
- 🟢 **PACK/PICK 모두 보이기 / 모두 숨기기** 버튼 추가 (카드 표시 일괄 토글)
- 🟢 **TC 포지션** — '한 장으로 보기' 종료 버튼 + ESC 지원, 포지션 설정 모달에 **검색**(닉네임/성함/쿠코드), 보드 디자인 가시성 개선
- 🟢 **수정 이력 표시 개선** — JSON 대신 `쿠코드 | 성함 | 조 · 6.3F 싱귤 ▶ 6.1F 싱귤` 처럼 사람이 읽기 좋게
- 🟢 사원 카드 **특이사항**을 모든 사용자가 입력 가능하도록 변경
- 🟢 그리드 **드래그 행 선택** (셀 위에서 끌면 자동 체크), DATA/FLOW 화면 디자인 정리

---

## 🔧 변경 로그 — v4 (대량 등록 + 화면 개편)

- 🟢 **DATA 엑셀 가져오기/내보내기/템플릿** — `📗 엑셀` 메뉴에서 카테고리별 .xlsx 템플릿 다운로드 → 작성 → 가져오기 (2,000명+ 일괄 등록, 중복 쿠코드는 마지막 행 우선, 허용 외 스킬 값 자동 무시)
- 🟢 **관리자 추가 기능** — 설정 → ⚡ 관리자에서 닉네임 등록/해제 (등록 즉시 해당 닉네임으로 관리자 입장 가능)
- 🟢 **로그인 · 조 선택 화면 전면 리디자인** — 글래스 카드 + 그라데이션 배경, DAY(해/구름)·SWING(달/별) 테마 카드, 현재 시간대 추천 배지, "다른 닉네임으로 입장" 버튼
- 🟢 **그리드 드래그 다중 선택** — 셀 아무 곳이나 누른 채 드래그하면 행이 자동 체크됨 (복사는 기존처럼 쿠코드→A열, 성함→B열 TSV)
- 🟢 **PACK/PICK 레이아웃 개편** — W/S 워터를 상단 띠에서 스트립 맨 오른쪽 카드로 이동, PICK 싱귤 아래에 멀티 수직 배치
- 🟢 이름 라벨의 ⭐ 하이스킬 배지 제거 (M/A/P 색 블록으로 일원화)
